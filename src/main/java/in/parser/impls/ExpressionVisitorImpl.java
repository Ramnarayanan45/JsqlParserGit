    ...
    public <S> QueryLayer visit(GreaterThanEquals greaterThanEquals, S context) {
        ...
        if (left instanceof Column column) {
            ...
            if (right instanceof LongValue ||
                    ...
                    right instanceof DateValue ||
                    right instanceof TimeValue ||
                    right instanceof TimestampValue || right instanceof BooleanValue) {
    ...
    public <S> QueryLayer visit(InExpression inExpression, S context) {
        ...
        if (left instanceof Column column) {
            ...
            if (right instanceof AndExpression) {
                // JSQLParser quirk: IN (list) AND cond1 AND cond2 ... is deeply nested:
                // InExpression{ right = AndExpression{ left = AndExpression{ left = list, right = ... }, right = ... } }
                // Recursively unwrap left side of AndExpression chain to find the actual IN list.
                // Collect all right-side expressions to visit as trailing conditions.

                // Step 1: unwrap to find the actual IN right (list or subquery)
                ...
                // node is now the actual IN right (ExpressionList, ParenthesedExpressionList, or Select)
                Expression actualRight = node;

                // Step 2: process the actual IN right
                ...
                // Step 3: visit all trailing conditions normally
